...
- **Чат с AI** — модель получает контекст (профиль, план, статистика) и отвечает. При намерении «добавить тренировку» в system prompt добавляются правила для блока ACTION; при подтверждении пользователя модель выводит блок `<!-- ACTION add_training_day ... -->` или JSON. Бэкенд парсит и выполняет действие.
- **Парсинг и выполнение** — в `ChatService::parseAndExecuteActions()`: из ответа извлекаются date, type, description; вызывается `WeekService::addTrainingDayByDate()`. Блок удаляется из сохраняемого текста. Поддерживаются форматы XML-подобный и JSON.
- **API** — `add_training_day_by_date` (WeekController / WeekService). Параметры: `date`, `type`, `description?`, `is_key_workout?`. Подробно — [ai-add-workouts-instruction.md](./ai-add-workouts-instruction.md).
...
Правила:
- Блок — в конце сообщения AI (или отдельная строка). Допускается также JSON: `{"action":"add_training_day","date":"...","type":"...","description":"..."}`.
- `date` — обязательно, формат Y-m-d.
- `type` — обязательно, из списка: rest, easy, long, tempo, interval, fartlek, other, sbu, race, control, marathon, free (соответствует `validTypes` в ChatService и WeekValidator).
- `description` — как в AddTrainingModal: для бега «X км, темп MM:SS» или «X км или ЧЧ:ММ:СС, темп MM:SS»; для интервалов «Разминка 2 км. 5×1000 м (4:00), пауза 400 м трусцой. Заминка 2 км.»; для ОФП «Упражнение — 3×10, 20 кг»; для СБУ «Упражнение — 30 м». Темп в формате MM:SS мин/км. Детальные форматы заданы в system prompt в `ChatService::buildChatMessages()` при `hasAddTrainingIntent`.
...
| Шаг | Описание | Статус |
|-----|----------|--------|
| 1 | Дополнить system prompt правилами для action block | ✅ buildChatMessages() при hasAddTrainingIntent |
| 2 | Реализовать `parseAndExecuteActions()` в ChatService | ✅ parseAndExecuteActions(), поддержка XML и JSON |
| 3 | Вызывать парсер после получения ответа, выполнять add_training_day | ✅ WeekService::addTrainingDayByDate() |
| 4 | Удалять action block из сохраняемого текста | ✅ блок вырезается перед сохранением |
| 5 | Подтверждение в ответе | ✅ текст модели уже содержит «добавил на …» |
