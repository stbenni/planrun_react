...
|---|---|---|---|
| 113 ✅ УДАЛЁН v3.27 | **`generate_plan_async.php` — 100% мёртвый** | 163 стр | systemd → `scripts/plan_generation_worker.php`, не этот файл. Re-verified 0 ссылок (require/exec/cron/service). Файл удалён `git rm`. |
...
| 117 ✅ УДАЛЕНО v3.27 | **Чистые orphan-функции** (0 ссылок везде, включая тесты) | — | `text_generator.php` удалён целиком (`git rm`, 0 require). `hasStructuredFields` вырезана из plan_normalizer.php. `parsePlanRunAIResponse` вырезана из plan_generator.php. Все re-verified 0 ссылок. |
...
| # | Что | Тяжесть | Детали |
|---|---|---|---|
| 123 ✅ УДАЛЕНО v3.27 | **3 мёртвых private-метода** | средне | `activateLatestPlan()`, `preflightSyncTargetIfUnrealistic()` вырезаны из PlanGenerationProcessorService; `classifyFeedback()` из PostWorkoutFollowupService. Re-verified 0 ссылок (incl callable/string). `enrichAggregateRow` оставлен — не мёртв (`array_map([$this,...])`). |
