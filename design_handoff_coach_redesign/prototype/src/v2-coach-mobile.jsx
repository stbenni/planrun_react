...
function CoachMobile() {
  ...
  return (
    <div style={CM.shell}>
      ...
      <div style={CM.nav}>
        {[
          ['Поток',     '◐', true, V2.EVENTS.filter(e => e.tone === 'danger' || e.kind === 'question').length],
          ['Команда',   '◈', false, null],
          ['Календарь', '▦', false, null],
          ['Чат',       '✉', false, 7],
          ['Меню',      '☰', false, null],
        ].map(([l, ic, on, badge]) => (
          <div key={l} style={CM.navItem}>
            <span style={{ position: 'relative', fontSize: 18, color: on ? CT.primary : CT.ink3 }}>
              {ic}
              {badge && <span style={CM.navBadge}>{badge}</span>}
            </span>
            <span style={{ fontSize: 9.5, color: on ? CT.ink : CT.ink3, fontWeight: on ? 700 : 500, marginTop: 2 }}>{l}</span>
          </div>
        ))}
      </div>
