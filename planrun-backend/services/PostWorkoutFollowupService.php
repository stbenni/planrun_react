...
class PostWorkoutFollowupService extends BaseService {
    ...
    private function buildFollowupPrompt(int $userId, array $summary): string {
        ...
        return "Как ты {$subject}? Опиши парой слов — как ноги, дыхание, есть ли где-то дискомфорт или боль. " .
            "Если хочется точнее, можно цифрами: тяжесть 1–10, ноги 1–10, дыхание 1–10, пульс 1–10, боль 0–10. " .
            "Главное — если что-то болит или чувствуешь перегруз, напиши: я учту это в следующем плане.";
    }
    ...
    private function buildCoachReply(int $userId, string $workoutDate, string $classification, ?array $summary): string {
        ...
        return match ($classification) {
            'pain' => trim("Понял про дискомфорт — записал. Не продавливай через боль: один лёгкий день обычно решает, неделя простоя — нет. {$nextAdvice} Скажи, если нужно облегчить или перенести следующую тренировку."),
            'fatigue' => trim("Усталость накопилась — это часть процесса, главное не игнорировать. Сейчас приоритет — восстановление: сон, белок, лёгкая активность. {$nextAdvice} Готов подкрутить ближайший день, если ощущение не пройдёт."),
            'good' => trim("Хороший знак — нагрузка усваивается. {$nextAdvice}"),
            default => trim("Записал. Если позже что-то даст о себе знать — боль, тяжесть, перегруз — пиши, скорректируем. {$nextAdvice}"),
        };
    }
    ...
    private function buildNextDayAdvice(?array $nextDay): string {
        if ($nextDay === null) {
            return 'Если что-то по восстановлению пойдёт не так — напиши, подкрутим план.';
        }
        ...
        if (in_array($type, ['rest', 'free'], true)) {
            return "Дальше по плану {$this->formatDateRu($date)} — {$typeRu}, используй его именно для восстановления.";
        }

        return "Следующая по плану {$this->formatDateRu($date)} — {$typeRu}. Если к ней не выровняешься, не форсируй, скажи мне.";
    }
